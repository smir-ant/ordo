...
live_design! {
    ...
    pub InputFlat = <InputBase> {
        ...
        draw_bg: {
            ...
            uniform base_color: #333
            uniform base_color_hover: #3a3a3a
            uniform base_color_focus: #3a3a3a
            uniform base_color_down: #2a2a2a
            uniform base_color_empty: #2a2a2a
            uniform base_color_disabled: #252525

            fn pixel(self) -> vec4 {
                let sdf = Sdf2d::viewport(self.pos * self.rect_size);
                
                // SUBTLE gradient like DOW (~10% difference for slight 3D inset effect - opposite of button)
                let base_color = mix(
                    mix(
                        mix(
                            mix(self.base_color, self.base_color_empty, self.empty),
                            self.base_color_focus,
                            max(self.focus, self.hover)
                        ),
                        self.base_color_down,
                        self.down
                    ),
                    self.base_color_disabled,
                    self.disabled
                );
                let fill_color = mix(base_color * 0.95, base_color * 1.05, self.pos.y);
                
                // Gradient stroke only: dark on top, light on bottom (inset/depth effect)
                let stroke_top = #222;
                let stroke_bottom = #444;
                let base_stroke = mix(stroke_top, stroke_bottom, self.pos.y);
                
                let active_stroke = mix(#333, #555, self.pos.y);
                let stroke_color = mix(
                    mix(
                        base_stroke,
                        active_stroke,
                        max(self.hover, self.focus)
                    ),
                    #ff0000,
                    self.warning * max(0.0, 1.0 - self.pos.y * 1.5)
                );
    ...
    // InputGradientX and InputGradientY removed - gradients not supported in simplified shader
    // Use Input instead
}
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        match event.hits(cx, self.draw_bg.area()) {
            ...
            Hit::KeyFocus(_) => {
                ...
                cx.widget_action(uid, &scope.path, InputAction::KeyFocus);
            },
            Hit::KeyFocusLost(_) => {
                self.handle_focus_lost(cx, &scope.path, uid);
                self.validate(cx);
            ...
            Hit::KeyDown(kev @ KeyEvent {
                ...
                if !did_move {
                    cx.widget_action(uid, &scope.path, InputAction::KeyDownUnhandled(kev));
                }
            }
            Hit::KeyDown(kev @ KeyEvent {
            ...
            Hit::KeyDown(kev @ KeyEvent {
                ...
                match self.move_cursor_up(cx, keep_selection) {
                    Ok(true) => { }
                    Ok(false) => cx.widget_action(uid, &scope.path, InputAction::KeyDownUnhandled(kev)),
            ...
            Hit::KeyDown(KeyEvent {
                ...
                cx.widget_action(uid, &scope.path, InputAction::Escaped);
            }
            Hit::KeyDown(KeyEvent {
                key_code: KeyCode::ReturnKey,
                modifiers: KeyModifiers {
                ...
                self.draw_bg.redraw(cx);
                cx.widget_action(uid, &scope.path, InputAction::Changed(self.text.clone()));
                self.validate(cx);
            }
            Hit::KeyDown(KeyEvent {
            ...
            Hit::KeyDown(event) => {
                cx.widget_action(uid, &scope.path, InputAction::KeyDownUnhandled(event));
            }
            _ => {}
        }
    }
...
impl Input {
    ...
    pub fn set_is_warning(&mut self, cx: &mut Cx, is_warning: bool) {
        self.is_warning = is_warning;
        if self.is_warning {
            self.animator_play(cx, ids!(warning.on));
        } else {
            self.animator_play(cx, ids!(warning.off));
        }
    }
    ...
    pub fn validate(&mut self, cx: &mut Cx) -> bool {
        if self.is_required {
            if self.text.is_empty() {
                self.set_is_warning(cx, true);
                return false;
            } else {
                self.set_is_warning(cx, false);
                return true;
            }
        }
        true
    ...
    fn handle_focus_lost(&mut self, cx: &mut Cx, scope_path: &HeapLiveIdPath, uid: WidgetUid) {
        ...
        cx.widget_action(uid, scope_path, InputAction::KeyFocusLost);
    }
...
impl InputRef {
    ...
    pub fn validate(&self, cx: &mut Cx) -> bool {
       if let Some(mut inner) = self.borrow_mut() {
           inner.validate(cx)
       } else {
           true
    ...
    pub fn escaped(&self, actions: &Actions) -> bool {
        for action in actions.filter_widget_actions_cast::<InputAction>(self.widget_uid()){
            if let InputAction::Escaped = action {
                return true;
            }
        }
        false
    ...
    pub fn changed(&self, actions: &Actions) -> Option<String> {
        for action in actions.filter_widget_actions_cast::<InputAction>(self.widget_uid()){
            if let InputAction::Changed(text) = action{
                return Some(text);
            }
        }
        None
    ...
    pub fn key_down_unhandled(&self, actions: &Actions) -> Option<KeyEvent> {
        for action in actions.filter_widget_actions_cast::<InputAction>(self.widget_uid()){
            if let InputAction::KeyDownUnhandled(event) = action{
                return Some(event);
            }
        }
        None
    ...
    /// to a new `InputState` object.
    pub fn save_state(&self) -> InputState {
        if let Some(inner) = self.borrow() {
            InputState {
    ...
    /// from the given `InputState` object.
    pub fn restore_state(&self, cx: &mut Cx, state: InputState) {
        if let Some(mut inner) = self.borrow_mut() {
